...
# --------------------------------------------------
#
# TransposeTensorProjection Layer (qk>pk)
#
# --------------------------------------------------

class TransposeTensorProjectionLayer(tf.keras.layers.Layer):
    def __init__(self, q1, q2, q3, **kwargs):
        super(TransposeTensorProjectionLayer, self).__init__(**kwargs)
        self.q1 = q1
        self.q2 = q2
        self.q3 = q3
    ...
    def get_config(self):
        base_config = super(TransposeTensorProjectionLayer, self).get_config()
        base_config['q1'] = self.q1
        base_config['q2'] = self.q2
        base_config['q3'] = self.q3
        return base_config
    ...
    def build(self, input_shape):
        ...
        if self.q1 > self.p1:
            self.W1 = self.add_weight(name="W1", shape=(self.q1, self.p1), initializer='normal', trainable=True) # q1xp1
        if self.q2 > self.p2:
            self.W2 = self.add_weight(name="W2", shape=(self.q2, self.p2), initializer='normal', trainable=True) # q2xp2
        if self.q3 > self.p3:
            self.W3 = self.add_weight(name="W3", shape=(self.q3, self.p3), initializer='normal', trainable=True) # q3xp3
            
        super(TransposeTensorProjectionLayer, self).build(input_shape)
    ...
    def WtoU(self, W):
        ...
        p = tf.shape(W)[1]
        Ip = tf.eye(p)
        WT = tf.transpose(W, perm=[1,0]) # pk x qk
        M = tf.math.add(tf.linalg.matmul(WT, W), Ip * e) # pk x pk
        sqrtM = tf.linalg.sqrtm(M)
        G = tf.linalg.inv(sqrtM) # pkxpk
        U = tf.linalg.matmul(W, G) # qk x pk
        return U
    ...
    def call(self, X):
        ...
        n = tf.shape(X)[0]
        if self.q1 > self.p1:
            U1 = self.WtoU(self.W1) # q1xp1
            Z = self.kmode_product(Z, U1, 1)
        if self.q2 > self.p2:
            U2 = self.WtoU(self.W2) # q2xp2
            Z = self.kmode_product(Z, U2, 2)
        if self.q3 > self.p3:
            U3 = self.WtoU(self.W3) # q3xp3
            Z = self.kmode_product(Z, U3, 3)
